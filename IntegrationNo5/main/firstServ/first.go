...
func main() {
	...
	mux.HandleFunc("/getFriends/", srv.GetUserFriends)
	mux.HandleFunc("/user/", srv.UpdateUserAge)

	http.ListenAndServe(":8080", mux)
}

func loadUsers(s *servie) {
	data, err := ioutil.ReadFile(s.file)
	if err != nil {
		fmt.Println("No users file found, starting with an empty store.")
		return
	}
...
func saveUsers(s *servie) {
	...
	if err != nil {
		fmt.Println("Error writing users file:", err)
	}
}
...
func (s *servie) GetUserFriends(w http.ResponseWriter, r *http.Request) {
	...
	if len(pathParts) < 3 || pathParts[1] != "getFriends" {
		w.WriteHeader(http.StatusNotFound)
